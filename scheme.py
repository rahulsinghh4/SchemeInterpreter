...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        val = scheme_eval(expressions.first, env)
        if is_true_primitive(val) or val is 0:
            return val
        else:
            if expressions.rest == nil:
                return val
...
def make_let_frame(bindings, env):
    ...
    while temp_bindings is not nil:
        validate_form(temp_bindings.first, 2, 2)
        x = temp_bindings.first.first
        names = Pair(x, names)
        validate_formals(names)
        f = temp_bindings.first.rest.first
        values = Pair(scheme_eval(f, env), values)
        temp_bindings = temp_bindings.rest
